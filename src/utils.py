...
def load_test_data() -> Dataset:
    ...
    Loads the dataset
    """
    df = pd.read_csv(DATA_DIR_SERVER + "central_test.csv") # the test data is on the central server
...
def load_data(agent_id = 0) -> Dataset:
    ...
    means = pd.read_csv(DATA_DIR + f"mean_{agent_id}.csv",index_col = 0)
    stds = pd.read_csv(DATA_DIR + f"std_{agent_id}.csv",index_col = 0)
